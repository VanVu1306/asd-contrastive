...
from models.heads import CentroidScoringHead, ClassificationHead, GMMScoringHead
...
def _write_selected_k_config(cfg, save_dir: str, chosen_k: int, sweep_report: str) -> str:
    """After a BIC sweep picks a K, bake it into a standalone copy of the
    resolved config (`eval.centroids.k` overwritten, `k_selection` reset to
    "fixed") so the choice survives without re-running the search — this is
    the "lưu vào file config cuối" step. Returns the written path."""
    import copy

    import yaml

    cfg_copy = copy.deepcopy(dict(cfg))
    cfg_copy["eval"] = copy.deepcopy(dict(cfg["eval"]))
    cfg_copy["eval"]["centroids"] = copy.deepcopy(dict(cfg["eval"]["centroids"]))
    cfg_copy["eval"]["centroids"]["k"] = chosen_k
    cfg_copy["eval"]["centroids"]["k_selection"] = "fixed"

    out_path = os.path.join(save_dir, "eval_config_selected_k.yaml")
    with open(out_path, "w") as f:
        f.write("# Auto-generated by eval.py's BIC-based K selection (eval.centroids.k_selection: \"bic\").\n")
        for line in sweep_report.splitlines():
            f.write(f"# {line}\n")
...
def run_anomaly_scoring(cfg, device: torch.device):
    ...
    # --- K selection: fixed (config value) or automatic via BIC sweep ---
    chosen_k = centroid_cfg["k"]
    k_selection = centroid_cfg.get("k_selection", "fixed")
    if k_selection == "bic":
        k_range = centroid_cfg.get("k_search_range") or list(range(2, 31, 2))
        time_budget = centroid_cfg.get("bic_time_budget_sec", 60.0)
        sweep = select_k_by_bic(
            normal_features, k_range, seed=centroid_cfg.get("kmeans_seed", 42), time_budget_sec=time_budget,
        )
        report = format_sweep_report(sweep)
        print(f"[eval] {report}")
        save_dir = os.path.dirname(centroid_cfg["save_path"]) or "."
        os.makedirs(save_dir, exist_ok=True)
        if sweep.best_k is not None and not sweep.timed_out:
            chosen_k = sweep.best_k
            config_path = _write_selected_k_config(cfg, save_dir, chosen_k, report)
            print(f"[eval] BIC-selected K={chosen_k} — used for this run and saved to {config_path}")
        else:
            print(
                f"[eval] WARNING: BIC sweep did not finish within {time_budget:.0f}s "
                f"(or found no valid candidate) — falling back to the configured k={chosen_k} for this "
                f"run. Run scripts/select_k_bic.py separately (no time limit) for a full sweep, then set "
                f"eval.centroids.k manually."
            )
    
    # --- Build & fit the scoring head (K-Means or GMM) ---
    method = centroid_cfg.get("method", "kmeans")
    save_dir = os.path.dirname(centroid_cfg["save_path"]) or "."
    os.makedirs(save_dir, exist_ok=True)
    
    if method == "gmm":
        scoring_head = GMMScoringHead(
            feature_dim=backbone.out_dim, k=chosen_k,
            mode=centroid_cfg.get("gmm_mode", "nll"),
            combined_weights=tuple(centroid_cfg.get("gmm_combined_weights", [0.5, 0.5])),
            entropy_temperature=centroid_cfg.get("gmm_entropy_temperature", 5.0),
        )
        scoring_head.fit(normal_features, seed=centroid_cfg.get("kmeans_seed", 42))
        torch.save(scoring_head.export_state(), centroid_cfg["save_path"])
        print(
            f"[eval] fit a {chosen_k}-component GMM (mode={scoring_head.mode}) from {len(normal_ds)} "
            f"normal clips -> {centroid_cfg['save_path']}"
        )
    elif method == "kmeans":
        scoring_head = CentroidScoringHead(feature_dim=backbone.out_dim, k=chosen_k)
        scoring_head.fit(normal_features, seed=centroid_cfg.get("kmeans_seed", 42))
        torch.save(scoring_head.centroids, centroid_cfg["save_path"])
        print(f"[eval] mined {chosen_k} centroids from {len(normal_ds)} normal clips "
              f"-> {centroid_cfg['save_path']}")
    else:
        raise ValueError(f"Unknown eval.centroids.method '{method}' (use 'kmeans' or 'gmm').")
